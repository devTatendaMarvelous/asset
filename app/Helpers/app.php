...
//    }
//}
function balance($crime)
{
    $payments  = $crime->payments()->sum('amount');
    $crimeAmount = $crime->crime->price;
    return $crimeAmount-$payments;
...
function generateSmsMessage($payment)
{
    $committed = $payment->crime;
    $crime= $committed->crime->name ;
    $crimeRef=$committed->reference;
    $offender=$committed->offender->name;
    ...
    return "Dear {$offender},
We have received your payment for the crime \"{$crime}\" (Ref: {$crimeRef}).
...
function generateCrimeSmsMessage($crime)
{

$date=parseDate($crime->occurrence_time);

$amount = number_format($crime->crime->price,00);

    $app = config('app.name');
    return "Dear {$crime->offender->name},
Your vehicle with plate number \"{$crime->vehicle->registration_number}\" has been issued a traffic fine of USD {$amount} for the offence \"{$crime->crime->category->name}\" on {$date}.


Failure to comply may lead to further penalties.
...
function resolveCrimeSignature($signature='')
{
    $url = env('CONTABO_URL') . 'storage/download?bucket=' . 'test' . '&filename=' . urlencode($signature);
